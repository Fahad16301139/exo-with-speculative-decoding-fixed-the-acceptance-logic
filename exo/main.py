...
from exo.models import get_repo, get_tokenizer_repo, build_base_shard, pretty_name
...
parser.add_argument("--max-tokens", type=int, default=50, help="Maximum number of tokens to generate (default: 50)")
parser.add_argument("--enable-speculative", action="store_true", help="Enable speculative decoding")
parser.add_argument("--draft-tokens", type=int, default=4, help="Number of draft tokens for speculative decoding")
parser.add_argument("--draft-model", type=str, help="Draft model for speculative decoding")
parser.add_argument("--speculative-threshold", type=float, default=0.8, help="Acceptance threshold for speculative decoding")
parser.add_argument("--max-speculation-depth", type=int, default=8, help="Maximum speculation depth")
...
# Create speculative config if enabled
...
if args.enable_speculative:
  ...
  speculative_config = SpeculativeConfig(
    ...
    max_speculation_depth=args.max_speculation_depth,
    adaptive_speculation=True
  )
  print(f"[Speculative] Enabled with {args.draft_tokens} draft tokens, threshold: {args.speculative_threshold}")
  if args.draft_model:
    print(f"[Speculative] Using draft model: {args.draft_model}")
  else:
    print(f"[Speculative] Auto-selecting draft model based on target model")
...
# Add periodic speculative metrics broadcasting
async def periodic_speculative_metrics():
  """Periodically broadcast speculative decoding metrics"""
  while True:
    try:
      await asyncio.sleep(5.0)  # Broadcast every 5 seconds
      if hasattr(node, 'broadcast_speculative_metrics'):
        await node.broadcast_speculative_metrics()
    except Exception as e:
      if DEBUG >= 1:
...
async def run_model_cli(node: Node, model_name: str, prompt: str):
  ...
  try:
    ...
    tokens = []
    max_tokens = getattr(args, 'max_tokens', 50)  # Get max_tokens from args, default to 50
    ...
    def on_token(_request_id, _tokens, _is_finished):
      if _request_id == request_id:
        ...
        if len(tokens) >= max_tokens:
          if DEBUG >= 1:
            print(f"[Token Limit] Reached max tokens limit: {len(tokens)}/{max_tokens}")
          return True  # Stop generation
      return _request_id == request_id and _is_finished
    
    await callback.wait(on_token, timeout=300)

    print(f"\nGenerated response ({len(tokens)} tokens):")
...
async def main():
  ...
  # Start periodic speculative metrics broadcasting if speculative decoding is enabled
  if args.enable_speculative:
    asyncio.create_task(periodic_speculative_metrics())
    if DEBUG >= 1:
      print("[Speculative] Started periodic metrics broadcasting")
