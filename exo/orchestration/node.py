...
class Node:
  ...
  def on_node_status(self, request_id, opaque_status):
    try:
      ...
      # Handle speculative metrics
      if status_type == "speculative_metrics":
        if DEBUG >= 2: print(f"Speculative metrics from {status_data.get('node_id')}: {status_data.get('metrics')}")
  ...
  async def broadcast_speculative_metrics(self):
    """Broadcast speculative decoding metrics if available"""
    try:
      from exo.inference.speculative.speculative_inference_engine import SpeculativeInferenceEngine
      if isinstance(self.inference_engine, SpeculativeInferenceEngine):
        metrics = self.inference_engine.get_metrics()
        status_message = json.dumps({
          "type": "speculative_metrics", 
          "node_id": self.id, 
          "metrics": metrics
        })
        await self.broadcast_opaque_status("", status_message)
    except ImportError:
      pass  # Speculative module not available
    except Exception as e:
      if DEBUG >= 1: print(f"Error broadcasting speculative metrics: {e}")